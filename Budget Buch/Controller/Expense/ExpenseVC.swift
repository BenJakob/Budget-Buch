...
class ExpenseVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func setupControls() {
        ...
        navigationItem.title = EXPENSES_TITLE
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if sortedEntries.count != 0 {
            ...
            performSegue(withIdentifier: TO_EDIT_EXPENSE, sender: nil)
        }
    }
    ...
    @objc private func addButtonPressed() {
        performSegue(withIdentifier: TO_ADD_EXPENSE, sender: nil)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let entryVC = segue.destination as? EntryDelegateVC else { return }
        if segue.identifier == TO_ADD_EXPENSE {
            (entryVC as EntryDelegate).configureAsNewExpenseVC()
        } else if segue.identifier == TO_EDIT_EXPENSE {
            guard let selectedEntry = selectedEntry else { return }
            (entryVC as EntryDelegate).configureAsEditExpenseVC(expense: selectedEntry as! Expense)
        }
    }
}
