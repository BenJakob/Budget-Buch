...
class NewIncomeVC: UIViewController, UITextFieldDelegate, NewEntryDelegate {
    ...
    override func viewDidLoad() {
        ...
        adBanner.adUnitID = NEW_INCOME_AD
    ...
    @objc private func safe(){
        ...
        if date.isEmpty || category.isEmpty || interval.isEmpty || amount.isEmpty {
            ...
            if categories.count > 0 {
                if interval == NONE {
                    DatabaseController.insertEntry(forEntityName: INCOME, as: .income, category: categories[0], date: date, amount: amount, interval: interval)
                } else {
                        ...
                        if let startDate = DateFormatter.customFormatter.date(from: date) {
                            ...
                            if startDate < currentDate! {
                                ...
                                alertController.addAction(UIAlertAction(title: AUTOFILL_YES, style: .default, handler: { (action) in
                                    if let newIncome = DatabaseController.insertRepeatingIncome(category: categories[0], startDate: date, endDate: self.endDateTextField.text, amount: amount, interval: interval) {
                                        DatabaseController.insertMissingRepeatingIncomes([newIncome], beginAt: startDate)
                                    }
